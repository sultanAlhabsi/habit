...
export const HomeScreen: React.FC<HomeScreenProps> = ({ navigation }) => {
  ...
  // ⚡ Bolt Optimization: Memoize expensive stats calculation that loops through all habit history.
  // This prevents UI thread blocking and frame drops when typing in search or switching tabs.
  ...
  const baseHabits = useMemo(
    () => (isSearchActive ? filterHabitsByQuery(activeUnarchivedHabits, searchQuery) : dueHabits),
    [isSearchActive, searchQuery, activeUnarchivedHabits, dueHabits]
  );
  ...
  const sortedFilteredHabits = useMemo(
    () => sortHabits(filteredHabits, sortOption, checkins, selectedDate),
  ...
  const offScheduleHabits = useMemo(
    () => activeUnarchivedHabits.filter((h) => !dueHabits.some((dh) => dh.id === h.id)),
